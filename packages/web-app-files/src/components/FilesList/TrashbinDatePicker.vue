...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const defaultStart = DateTime.now().setLocale(language.current).minus({ days: 2 }).toJSDate()
    ...
    // // Push the default value so that we always have to/from set (backend requires it)
    // const query = { ...router.currentRoute.value.query }
    // query.from = defaultStart.toISOString().slice(0, 10)
    // query.to = dateNow.value.toISOString().slice(0, 10)
    // router.push({ query })
