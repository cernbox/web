...
const navItems = [
  ...
  },
  {
    ...
    enabled(capabilities) {
      return (
        !window.__$store.getters.user.isLightweight && capabilities.files && capabilities.files.favorites
      )
    }
  },
  {
    name(capabilities) {
      return window.__$store.getters.user.isLightweight ? $gettext('Shared with me') : $gettext('Shares')
    },
    icon: 'share-forward',
    route: {
  ...
  },
  {
    ...
    enabled(capabilities) {
      return (
        !window.__$store.getters.user.isLightweight &&
        capabilities.dav &&
