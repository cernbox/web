...
describe('InviteCollaboratorForm', () => {
  ...
  describe('pasting a list of emails', () => {
    it('delegates to resolveEmailList when the search input looks like a pasted email list', () => {
      const { wrapper } = getWrapper()
      const resolveSpy = vi.spyOn(wrapper.vm, 'resolveEmailList').mockResolvedValue(undefined)

      wrapper.vm.onSearch('Jane Doe <jane@example.com>; John Doe <john@example.com>')

      expect(resolveSpy).toHaveBeenCalledWith(['jane@example.com', 'john@example.com'])
    })
    it('keeps the normal live-search behavior for a plain, typed query', () => {
      const { wrapper } = getWrapper()
      const resolveSpy = vi.spyOn(wrapper.vm, 'resolveEmailList')
