  ...
  watch(
    ...
    async ([driveAliasAndItem, areSpacesLoading], [driveAliasAndItemOld, areSpacesLoadingOld]) => {
      ...
      try {
        ...
        // swallowed on purpose: a watcher callback has no caller to propagate to, so rethrowing
        // only produces an unhandled rejection. The space simply stays unresolved.
        console.error(e)
      } finally {
        loading.value = false
      }
