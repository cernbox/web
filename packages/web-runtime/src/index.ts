...
export const bootstrap = async (configurationPath: string): Promise<void> => {
  ...
  const themePromise = announceTheme({ store, vue: Vue, designSystem, runtimeConfiguration })
  announceClientService({ vue: Vue, runtimeConfiguration })
  await announceClient(runtimeConfiguration)
  announceAuthService({ vue: Vue, configurationManager, store, router }) // Requires client
